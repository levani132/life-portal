...
function LoanBlock({ detail, today }: { detail: LoanDetail; today: string }) {
  ...
  const [recording, setRecording] = useState<{ amountCents?: number } | null>(
    null,
  );
    ...
    <div className="card overflow-hidden">
      <div className="border-b border-border p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              {loan.interestRate > 0 && (
                <Chip tone="warn">
                  {(loan.interestRate * 100).toFixed(2)}% a year
                </Chip>
              )}
            </div>
            {loan.label && (
              <p className="mt-1 text-sm text-ink-muted">{loan.label}</p>
            )}
          </div>
        ...
        <div className="mt-5 grid grid-cols-2 gap-4 sm:grid-cols-4">
          <Stat
            label="Outstanding"
            value={formatCents(detail.remainingCents, currency)}
            tone="warn"
          />
          <Stat
            label="Repaid"
            value={formatCents(detail.paidCents, currency)}
            tone="good"
          />
          <Stat
            label="Original"
            value={formatCents(loan.principalCents, currency)}
          />
          <Stat
            label="Since"
            ...
            hint={
              loan.targetPayoffDate
                ? `target ${formatDay(loan.targetPayoffDate)}`
                : undefined
            }
          />
        </div>
        ...
        <div className="mt-4">
          <div className="mb-1.5 flex justify-between text-xs text-ink-faint">
            ...
            <span>
              {formatCents(detail.paidCents, currency)} of{' '}
              {formatCents(loan.principalCents, currency)}
            </span>
          </div>
        ...
        {detail.unrecordedScheduledCents > 0 && (
          <div className="mt-4 rounded-lg border border-amber-500/40 bg-amber-500/10 px-3 py-2.5">
            <p className="text-xs leading-relaxed text-amber-100">
              ...
              . Your budget has already spent that money, so if it went out as
              planned you owe{' '}
              <span className="font-medium">
            ...
            <button
              ...
              onClick={() =>
                setRecording({ amountCents: detail.unrecordedScheduledCents })
              }
            >
              Record {formatCents(detail.unrecordedScheduledCents, currency)} as
              paid…
            </button>
          </div>
        )}
      ...
      <div className="grid gap-5 p-5 lg:grid-cols-2">
        ...
        <div className="space-y-5">
          <Panel
            title="How this gets repaid"
            description="Where each repayment is expected to come from."
          >
            {detail.plans.length === 0 ? (
              ...
              <ul className="space-y-2">
                {detail.plans.map((plan) => {
                  const inflow = detail.inflows.find(
                    (i) => i.planId === plan.id,
                  );
                  return (
                    <li
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <p className="mt-0.5 text-xs text-ink-faint">
                          {plan.kind === 'recurring' &&
                            `monthly on the ${plan.dayOfMonth ?? 1}`}
                          {plan.kind === 'one_off' &&
                            plan.date &&
                            `once on ${formatDay(plan.date)}`}
                          {plan.kind === 'items' &&
                            'from items earmarked for this debt'}
                          {plan.kind === 'stocks' &&
                            'from shares at their target price'}
                      ...
                      <div className="shrink-0 text-right">
                        ...
                        <button
                          ...
                          onClick={() =>
                            void run(() =>
                              api.delete(`/loans/plans/${plan.id}`),
                            )
                          }
                        >
                          remove
          ...
          <Panel
            ...
            description={`${detail.payments.length} recorded`}
          >
                  ...
                  >
                    ...
                    <div className="flex shrink-0 items-center gap-2">
                      ...
                      <button
                        ...
                        onClick={() =>
                          void run(() =>
                            api.delete(`/loans/payments/${payment.id}`),
                          )
                        }
                      >
                        ✕
  ...
  return (
    ...
    >
      ...
      <Field
        ...
        hint="The original amount, not what is left. Repayments are recorded in this currency."
      >
        <MoneyInput
          required
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Field label="Hope to clear by">
          <Input
            ...
            onChange={(e) =>
              setForm({ ...form, targetPayoffDate: e.target.value })
            }
          />
        </Field>
      </div>
      ...
      onSubmit={async () => {
        const ok = await run(() =>
          api.post(`/loans/${loanId}/plans`, {
            ...
            createLinkedExpense:
              kind === 'recurring' ? createLinkedExpense : undefined,
          }),
        );
        if (ok) onClose();
      }}
    >
      <Field label="Kind">
        <Select value={kind} onChange={(e) => setKind(e.target.value)}>
          ...
          <option value="stocks">
            Whatever the shares are worth at target
          </option>
        </Select>
      </Field>
      ...
      {derived ? (
        <p className="rounded-lg border border-border px-3 py-2 text-xs text-ink-muted">
          ...
          {kind === 'items'
            ? 'items you have earmarked for this debt'
            : 'share lots earmarked for this debt, at their target price'}
        ...
        <>
          ...
          <label className="flex items-start gap-2 rounded-lg border border-border px-3 py-2">
            ...
            <span className="text-xs text-ink-muted">
              Also add this to my budget, so it shows up in Free money and I can
              adjust the amount from either screen.
            </span>
          </label>
        </>
      )}
